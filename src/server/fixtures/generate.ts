...
export function buildExtractedFields(
  ...
  // A clean document: everything legible, confidence comfortably above the
  // review threshold.
  if (!degraded) {
    const fields = {} as Record<ExtractedFieldKey, FieldValue>;
    for (const key of EXTRACTED_FIELD_KEYS) {
      fields[key] = ok(source[key], floatBetween(rng, 0.88, 0.995));
    }
    return fields;
  }
  ...
  for (const key of EXTRACTED_FIELD_KEYS) {
    ...
    if (!damaged.has(key)) {
      fields[key] = ok(value, floatBetween(rng, 0.84, 0.98));
      continue;
    }
